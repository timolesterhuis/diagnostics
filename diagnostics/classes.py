...
class TimeSerie(object):
    ...
    def __init__(self, data, t0=0, name="", fs=1):
        ...
        self.mod_func = {'default': lambda x: x,
                         'zero_negatives': lambda x: x*(x>0),
                         'correct_negatives': lambda x: x - x*(x<0)}

    def __repr__(self):
        return "TimeSerie({}, t0={}, name={}, fs={})".format(self.data, *map(repr, [self.t0, self.name, self.fs]))
    ...
    def __eq__(self, other):
        if isinstance(other, TimeSerie):
            if self.channel != other.channel:
                raise ValueError("TimeSerie channels inconsistent! ({} & {})".format(self.channel, other.channel))
            return BooleanTimeSerie(self.data == other.data, t0=self.t0, fs=self.fs, name="({} == {})".format(self.name, other.name))
        else:
    ...
    def __add__(self, other):
        if isinstance(other, TimeSerie):
            if self.channel != other.channel:
                raise ValueError("TimeSerie channels inconsistent! ({} & {})".format(self.channel, other.channel))
            return TimeSerie(self.data + other.data, t0=self.t0, fs=self.fs, name="({} + {})".format(self.name, other.name))
        else:
    ...
    def __radd__(self, other):
        if isinstance(other, TimeSerie):
            if self.channel != other.channel:
                raise ValueError("TimeSerie channels inconsistent! ({} & {})".format(other.channel, self.channel))
            return TimeSerie(other.data + self.data, t0=other.t0, fs=other.fs, name="({} + {})".format(other.name, self.name))
        else:
    ...
    def __and__(self, other):
        ...
        if not (self.is_bool() and other.is_bool()):
            raise ValueError("Can't perform bitwise operation on non-boolean TimeSeries!")
        return BooleanTimeSerie(self.data & other.data, t0=self.t0, name="({} & {})".format(self.name, other.name), fs=self.fs)
    ...
    def __rand__(self, other):
        ...
        if not (self.is_bool() and other.is_bool()):
            raise ValueError("Can't perform bitwise operation on non-boolean TimeSeries!")
        return BooleanTimeSerie(other.data & self.data, t0=other.t0, name="({} & {})".format(other.name, self.name), fs=other.fs)
    ...
    def __rxor__(self, other):
        if self.channel != other.channel:
            raise ValueError("TimeSerie channels inconsistent! ({} & {})".format(other.channel, self.channel))
        if not all(self.is_bool(), other.is_bool()):
            raise ValueError("Can't perform bitwise operation on non-boolean TimeSeries!")
        return BooleanTimeSerie(other.data ^ self.data, t0=other.t0, name="({} ^ {})".format(other.name, self.name), fs=other.fs)

    def __invert__(self):
        if not self.is_bool():
            raise ValueError("Can't perform bitwise operation on non-boolean TimeSerie!")
        return BooleanTimeSerie(~self.data, t0=self.t0, name="(~{})".format(self.name), fs=self.fs)

    @classmethod
    def empty(cls, t0, te, fs, name=''):
        if isinstance(t0, datetime.datetime):
            t0 = t0.timestamp()
    ...
    def to_channel(self, c):
        if self.fs != c.fs:
            raise ValueError("can't modify channel to a different fs! please interpolate first")
        if c.t0 > self.t0:
            raise DataLossError("channel data window does not fully overlap! (c.t0 > self.t0)")
        if c.te < self.te:
            raise DataLossError("channel data window does not fully overlap! (c.te < self.te)")
    ...
    def modify(self, method, inplace=False):
        if inplace:
            ...
            return self.mod_func[method](self.data)
    ...
    def to_bool(self, inplace=False):
        ...
        if inplace:
            ...
            return BooleanTimeSerie(self.data, t0=self.t0, fs=self.fs, name=self.name)
...
class BooleanTimeSerie(TimeSerie):
    ...
    def __repr__(self):
        return "BooleanTimeSerie({}, t0={}, name={}, fs={})".format(self.data, *map(repr, [self.t0, self.name, self.fs]))
...
class StateChangeArray(object):
    ...
    def __repr__(self):
        return "StateChangeArray({}, t={}, name={})".format(self.data, self.t, repr(self.name))
...
class Event(object):
    ...
    def __repr__(self):
        return "Event({}, t={}, name={})".format(*map(repr, [self.value, self.t, self.name]))
